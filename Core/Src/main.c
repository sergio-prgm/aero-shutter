...
void setStepperOutput(uint8_t step)
{
...
void runStepperSeconds(uint32_t delay_seconds)
{
  uint32_t startTime = HAL_GetTick();
  uint32_t delay = delay_seconds * 1000;
  while (HAL_GetTick() - startTime < delay)
  {

    for (int i = 0; i < 8; i++)
    {
      if (HAL_GetTick() - startTime >= delay)
        break;
      setStepperOutput(stepSequence[i]);
      HAL_Delay(5);
    }
  }
  disableStepper();
...
int main(void)
{
  ...
  lightQueueHandle = osMessageQueueNew(5, sizeof(uint16_t), &lightQueue_attributes);
...
void StartLigthtSensorTask(void *argument)
{
  ...
  {
    ssd1306_Init();
    adc_value = ReadLightSensor();

    lightPercentage = (adc_value * 100) / 4096;
    osStatus_t status = osMessageQueuePut(lightQueueHandle, &lightPercentage, 0, osWaitForever);
    char buffer[4];
    sprintf(buffer, "%d", lightPercentage);
...
void StartMotorTask(void *argument)
{
  ...
  for (;;)
  {
    osStatus_t status = osMessageQueueGet(lightQueueHandle, &lightPercentage, NULL, osWaitForever);
